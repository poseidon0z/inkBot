...
from discord_components import component, interaction
import youtube_dl
...
class Music(commands.Cog):
    ...
    async def play_(self, ctx, *, search: str):
        ...
        if search.startswith("https://open.spotify.com/playlist/"):
            search_terms = []
            playlist = sp.user_playlist("spotify",search)
            for item in playlist['tracks']['items']:
                song_name = str(item['track']['name'])
                artist = ', '.join([artist['name'] for artist in item['track']['artists']])
                search_terms.append(song_name + " " + artist)
            for term in search_terms:
                source = await YTDLSource.create_source(ctx, term, loop=self.bot.loop, download=False,spotify_playlist=True)
                await player.queue.put(source)
            emb = discord.Embed(title="", description=f"Queued {len(search_terms)} songs", color=discord.Color.green())
